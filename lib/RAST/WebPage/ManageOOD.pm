package RAST::WebPage::ManageOOD;
...
sub output {
  ...
  if ( ref $self->data('categories') and @{  $self->data('categories') } > 0){
    ...
    $tab_view->add_tab('Add Categories', $self->display_edit_category) if ($cgi->param('task') eq "add_category");
    $tab_view->add_tab('Edit data fields for ' . $self->data('active_node') , $self->display_datafields) if ($cgi->param('task') eq "edit_datafields");
...
sub display_entries{
  ...
  if ( $self->data('task') eq "add_entry" ){
    ...
    if ( $self->data('active_node') ){
      ...
      $content .= "<li> <button type=\"submit\" name=\"task\" value=\"edit_datafields\">Edit</button> data fields for ". $self->data('active_node');
...
sub add_entry{
  ...
  #$self->app->add_message('warning' , "Adding entry " . $entry . " to " . $self->data('active_category'));
