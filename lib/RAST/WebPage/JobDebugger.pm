package RAST::WebPage::JobDebugger;
...
sub output {
  ...
  $content .= "<p> &raquo <a href='?page=GenomeStatistics&job=".$job->id."'>Genome Comparison with an earlier RAST version</a></p>" if (-f $job->org_dir."/comparison");
  ...
  for my $f (<$job_dir/rp.errors/C:*>)
  {
      my $b = basename($f);
      my $dir;
      opendir($dir, $f);
      my @x = grep{ ! /^\./ } readdir($dir);
      closedir($dir);
      push(@files, map { "$b/$_" } @x);
  }
...
sub load_file {
  ...
  my $content = "<p> &raquo <a href='?page=JobDebugger&job=".
