...
#
# Compute signal peptides and transmembrane domains of proteins using PHOBIUS server and create attribute data.
#
...
my @apps = qw(rp_PHOBIUS_search
	      rp_make_PHOBIUS_attributes
	      rp_index_attributes
	     );

$meta->set_metadata("phobius.hostname", $hostname);
...
$meta->add_log_entry($0, "finish phobius computation on $jobdir");
$meta->set_metadata("status.phobius", "complete");
exit(0);
