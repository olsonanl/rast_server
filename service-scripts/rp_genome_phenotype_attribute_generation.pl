...
#
# creates genome phenotype attribute data.
#
...
my @apps = qw(rp_make_genome_phenotype_attributes
	      rp_index_attributes
	     );

$meta->set_metadata("phenotypes.hostname", $hostname);
...
$meta->add_log_entry($0, "finish phenotypes computation on $jobdir");
$meta->set_metadata("status.phenotypes", "complete");
exit(0);
...
sub fatal
{
	...
	$meta->set_metadata("status.phenotypes", "error");
    }
