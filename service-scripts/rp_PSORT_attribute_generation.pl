...
#
# Run PSORT and create attributes data.
#
...
my @apps = qw(rp_run_PSORT_on_genome
	      rp_make_PSORT_attributes
	      rp_index_attributes
	     );

$meta->set_metadata("psort.hostname", $hostname);
...
$meta->add_log_entry($0, "finish psort computation on $jobdir");
$meta->set_metadata("status.psort", "complete");
exit(0);
...
sub fatal
{
	...
	$meta->set_metadata("status.psort", "error");
    }
