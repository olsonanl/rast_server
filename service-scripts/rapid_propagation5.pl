...
elsif (0 == &pull_orfs_with_kmers($keep, $meta,$figV,$sapO,$annoO, $taxonID,$code, $chkD,$tmpD,$restartD,$errD, $kmerDataset)) {
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
sub pull_orfs_with_kmers {
    my ($keep, $meta,$figV,$sapO,$annoO, $taxonID,$code, $chkD,$tmpD,$restartD,$errD, $errfile_name, $kmerDataset) = @_;
    my $msg;
    
    my $contigs_file = qq($tmpD/contigs);
    my $tmp_orf_pep  = qq($tmpD/orf_pep.tmp);
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#...Run the fragments, plasmid. or phage sequences through `pull_orfs` to
#   Create training data
#-----------------------------------------------------------------------------
    if (! $meta->get_metadata('status.rp.pull_orfs_with_kmers')) {
	if ($keep) {
	    #...Do nothing --- Annotations will be handled by &post_process_orfs();
	}
	else {
	    $msg = q(Starting pull_orfs);
	    warn ($msg, qq(\n)) if $ENV{VERBOSE};
	    &log($tmpD, $msg);
	    
	    my $pull_orfs_pipe;
	    open($pull_orfs_pipe, qq($FIG_Config::bin/pull_orfs 30 < $contigs_file | tee $tmp_orf_pep | svr_assign_using_figfams |))
		|| die qq(Could not pipe-out open pull_orfs via svr_assign_using_figfams);
	    
	    my @kmer_hits = <$pull_orfs_pipe>;
	    #...Trap errors once I figure out how...
	    
	    close($pull_orfs_pipe);
	    
	    $msg = q(pull_orfs completed, Kmer hits called);
	    warn ($msg, qq(\n)) if $ENV{VERBOSE};
	    &log($tmpD, $msg);
	    
	    if (@kmer_hits == 0) {
		$msg = q(No ORFs with Kmer hits);
		warn ($msg, qq(\n)) if $ENV{VERBOSE};
		&log($tmpD, $msg);
		
		return 1;   #...Nonzero retval indicates error !!!
	    }
	    
	    my %pep_of = map { ($_->[0] => $_->[2]) } &gjoseqlib::read_fasta($tmp_orf_pep);
	    
	    foreach my $hit (@kmer_hits) {
		chomp $hit;
		my ($score, $loc, $func) = split /\t/, $hit;
		print STDERR (join(qq(\t), ($score, $loc, $func)), qq(\n))
		    if ($ENV{DEBUG} && defined($ENV{VERBOSE}) && ($ENV{VERBOSE} > 1));
		
		my $pep = $pep_of{$loc};
		$pep =~ s/\*$//o;
		
		if (my $fid = $figV->add_feature(q(master:rast), $taxonID, q(orf), $loc, q(), $pep, qq($tmpD/called_by), q(pull_orfs_with_kmers))) {
		    $figV->assign_function($fid, q(master:rast), $func, q(), qq($tmpD/assigned_functions));
		}
		else {
		    die (q(Could not add ORF: ), join(qq(\t), ($loc, $score, $func, $pep)));
		}
	    }
 	}
    }
    
    unlink($tmp_orf_pep) unless $ENV{DEBUG};
    
    &make_restart($chkD, $tmpD, $restartD,  qq(pull_orfs_with_kmers));
    $meta->set_metadata('status.rp.pull_orfs_with_kmers',1);
...
sub correct_frameshifts {
	...
	if (! $meta->get_metadata('status.rp.correct_frameshifts')) {
	    if ($keep) {
		#...Annotate FS only...
		&my_run("correct_frameshifts -nofatal -code=$code $tmpD -just_mark > $errD/correct_frameshifts.stderr 2>&1");
	    }
	    else {
