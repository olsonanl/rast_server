...
#
# creates similar_to_human attribute data.
#
...
my @apps = qw(rp_run_blast_against_human
	      rp_make_similar_to_human_attributes
	      rp_index_attributes
	     );

$meta->set_metadata("sim2human.hostname", $hostname);
...
$meta->add_log_entry($0, "finish sim2human computation on $jobdir");
$meta->set_metadata("status.sim2human", "complete");
exit(0);
...
sub fatal
{
	...
	$meta->set_metadata("status.sim2human", "error");
    }
