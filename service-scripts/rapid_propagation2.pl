...
if (! $meta->get_metadata('status.rp.find_genes_based_on_kmers')) {
    &run("$bin/find_genes_based_on_kmers $tmpD $tmpD/found $old > $errD/find_genes_based_on_kmers.stage-1.stderr 2>&1");
    ...
    {
	&run("$bin/find_genes_based_on_kmers $tmpD $tmpD/found $old > $errD/find_genes_based_on_kmers.stage-2.stderr 2>&1");
